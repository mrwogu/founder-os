...
entry points, and examples. `.promptscript/agents.prs` holds all role
definitions.
...
|---|---|
| Current version | `.claude-plugin/marketplace.json`, both package manifests, and `.promptscript/project.prs`; all four must match. |
...
1. Bump the version in `.claude-plugin/marketplace.json`, both package
   manifests, and `.promptscript/project.prs`.
