...
portable identity, contracts, hooks, MCP server, plugin capability, workflow,
and examples. `.promptscript/agents.prs` holds all role definitions.
`.promptscript/skills/*/SKILL.md` is auto-discovered by PromptScript; nested
`agents/openai.yaml` files are imported as managed resources, so Claude and
Codex keep one skill source.
...
CI compiles and checks PromptScript before the six package gates on every push
and PR (`.github/workflows/ci.yml`). A red build is a no from the machine
...
3. Add `<name>` to the owning role's `skills` list in
   `.promptscript/agents.prs`. That agent must own every path in
   `metadata.writes`.
...
1. Update the role in `.promptscript/agents.prs`: `description`, `skills` (must
   include the three universal ones), and `tools` (an explicit allowlist -
   never omit it, and never an outbound tool).
